...
The California-based tech giant 's two-step verification initially covered iTunes and App Store purchases , but has now been extended to cover iCloud backups . When two-step verification is enabled , users will now need to enter a code to access their account - however , the Find My Phone feature will remain open -LRB- pictured -RRB-

The code is n't needed when a backup takes place , because the information is coming directly from a trusted device - it is only needed when users try to access their iCloud account from a web browser .
...
It is assumed that if a user does n't have their trusted device , they ca n't receive the four-digit verification code .

Two-step verification is an optional security feature , and settings are managed through the My Apple ID page .

Two-step verification , also called two-factor authentication , requires a user to have two of
...
At the start of the month , Mr Cook told the Wall Street Journal Apple will ` aggressively encourage
users to take stricter security measures ' , and the firm will also alert users if someone tries to access their files .
...
The code is sent to a registered , trusted device -LRB- pictured -RRB- . If users no longer have access to a trusted device , they can use a recovery key to add a new one . This key is issued during the two-step verification setup process

The changes come following the recent celebrity photo scandal that saw hundreds of images stolen from Apple 's iCloud service . Chief executive Tim Cook -LRB- pictured -RRB- recently said Apple will also start notifying users each time someone tries to access their account
...
Jennifer Lawrence -LRB- pictured -RRB- was among the 101 celebrities who had their photos stolen from iCloud . She is said to have posted more than 60 nude selfies to the online service
...
iCloud secures data by encrypting it when it is sent over the web , storing it in an encrypted format when kept on server , and using secure tokens for authentication -LRB- explained above -RRB- . This suggests the hackers were able to obtain the login credentials of the accounts , and pretend to be the user , in order to bypass this encryption
