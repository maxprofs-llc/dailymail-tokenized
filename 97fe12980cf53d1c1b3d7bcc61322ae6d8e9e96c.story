...
Sydney-based company is preparing to release a unified smart home device . The Ninja Sphere links different appliances in a home and controls them all . The central hub called a ` Spheramid ' -LRB- shown -RRB- can be controlled by smartphones or gestures
...
- Create ` if this then that ' -LRB- IFTTT -RRB- style rules against presence , location or time such as : ` If I 'm on my way home and it 's cold outside turn on the heater . '
...
Using GPS , Zigbee , Wi-Fi and Bluetooth LE -LRB- Low Energy -RRB- it is able to detect devices and create a ` map ' of them in a home .

Through an app it can then sync with a phone or other devices to control them all .

The brain of the entire system is the Spheramid , # 180 -LRB- $ 305 -RRB- , which acts as a central hub for different devices in the home .
...
One of the touted benefits is also that , using tags on certain items , it can detect when they are being moved and send an alert to the user -LRB- shown -RRB- . This could indicate than an intruder is in the home , for example
...
It 's not especially cheap at the moment , however : It costs # 180 -LRB- $ 305 -RRB- for one Spheramid , in addition to the other sensors that are needed .
...
Tiny devices known as a Waypoints also need to be placed around the house , which will allow the Ninja Sphere to locate things . This means , for example , it could show you where pets are in the house -LRB- shown -RRB- if they have a small Bluetooth-enabled collar . Ninja Sphere uses an open source modified version of Ubuntu Linux to operate
