...
` This cloak bends light and sends it through the center of the device , so the on-axis region can not be blocked or cloaked , ' said Choi .
...
By aligning four lenses in a certain manner an object is made ` invisible ' . The background behind the object remains visible -LRB- shown -RRB- as light bends around . The scientists say it is the first cloaking device to provide multidirectional cloaking . And the technique could also be used to let surgeons see through their hands

To test the device , which cost about # 600 -LRB- $ 1,000 -RRB- to put together the researchers placed an object in front of a grid background .
...
To make the device four lenses need to be aligned at a precise length -LRB- illustration shown -RRB- . The lenses need to be paired up and separated by the sum of their focal lengths - which is the distance from the lens that an object comes into focus . Doing this will make objects that are placed between the third and fourth lens invisible
