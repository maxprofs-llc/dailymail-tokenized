...
SSL 3.0 , in which the software is found , is currently used by Internet Explorer -LRB- IE -RRB- 6 .

Some other browsers all still l support SSL 3.0 as an option , so they may also be at risk .

To prevent a Poodle attack on Firefox , open about.config , search for ` security.enable , ' and set ` security.enable _ ssl3 ' to ` false ' , advises Steven Vaughan-Nichols writing in ZDNet .
...
SSL 3.0 , in which the software is found , is currently used by Internet Explorer -LRB- IE -RRB- 6 -LRB- right -RRB- . ` If Shellshock and Heartbleed -LRB- left -RRB- were Threat Level 10 , thenPoodle is more like a 5 or a 6 , ' said Tal Klein , vice president with cloud security firm Adallom
...
Web users can opt switch to using a browser that does n't use SSL 3.0
