...
If you are not connected when you receive a message for example -LRB- in its messaging security application -RRB- , then you will be invited to join the key management service .
...
` What we have developed is an algorithm that places a security layers into the DNA of the internet - it will have crypto for the masses , and you wo n't even need a special app . '
...
The simple Application Programming Interface -LRB- API -RRB- allows for any service provider to plug in any application to take care of privacy and security settings for data and image and voice .

This has never previously been possible .

An example of two such reference implementations include : Messaging security -- for cross platform on IPhone , Android and Blackberry where all SMS , MMS and emails -LRB- with attachments -RRB- are sent and received with just a simple one-click go button within the native applications -- so the user has no need to visit other programs or applications to secure content .
...
Professor Anthony Finkelstein , Dean of the Faculty of Engineering Sciences at UCL and a Fellow of the Royal College of Engineering , a member of Scentrics ' Technical Advisory Board -LRB- TAB -RRB- said : ` What differentiates low and high value IP is algorithmic research or its absence .
...
` In short , every individual and/or machine now has the ability to call , from the cloud , its own cryptographic key -LRB- pair -RRB- . This process , for the first time in history , is fully automated . '
