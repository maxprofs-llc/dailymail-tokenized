...
Using transcriptions of these monkey calls gathered in field experiments involving playbacks of predator calls -LRB- e.g. eagle shrieks and leopard growls -RRB- , the researchers found greater complexity in expression than previously understood as well as differences in alarm calls between the two locations .

Their analysis showed that these calls make a distinction between roots -LRB- especially ` hok ' and ` krak ' -RRB- and suffixes -LRB- - oo -RRB- , and that their combination allows the monkeys to describe both the nature of a threat and its degree of danger .
...
It posits that the meaning of a word can be enriched when it competes with a more informative alternative -- for instance , ` possible ' competes with ` certain , ' which is more informative , and for this reason ` possible ' usually comes to mean ` possible but not certain ' -LRB- for instance in : ` It 's possible that John is the culprit ' -- which implies that this is not a certainty -RRB- .

The authors propose that ` krak ' always has a meaning of general alarm , but that in Tai it comes to be enriched by competition with ` hok ' -LRB- meaning : aerial threat -RRB- and ` krak-oo ' -LRB- meaning : weak threat -RRB- -- with the result that it is enriched with a ` not ` hok ' ' component -LRB- hence : the threat is a non-aerial threat -RRB- and a ` not ` krak-oo ' ' component -LRB- hence : the threat is not weak -RRB- .
