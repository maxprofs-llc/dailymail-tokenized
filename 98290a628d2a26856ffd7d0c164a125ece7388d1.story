...
The California firm 's latest patent reveals plans to add customisable security options that could , based on a user 's location , automatically unlock a phone when the user is in a ` safe ' zone . For example , if the device is connected to the user 's home Wi-Fi network -LRB- pictured -RRB-
...
Additionally , the patent describes how the ` location ' does n't need to refer to a fixed location , and could also include a user 's car .
...
Additionally , the patent describes how the location does not need to refer to a fixed place , and could also include a user 's car . To verify the location , the phone could connect to the car 's Bluetooth , navigation systems or audio displays -LRB- pictured -RRB-

Alternatively , if the phone is on a public network , security levels will be increased , in addition to the PIN , passcode or fingerprint scan -LRB- scanner pictured -RRB- . The patent , titled ` Location-sensitive security levels and setting profiles based on detected location ' was filed in December 2012 and awarded earlier this month
...
` Security requirements help ensure that a mobile device is in the hands of the appropriate party , -LSB- but -RSB- often the security level remains the same regardless of the location of the mobile device .
