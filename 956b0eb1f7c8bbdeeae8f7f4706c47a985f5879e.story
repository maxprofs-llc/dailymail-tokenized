A ` master key ' that could give hackers complete access to mobile phones and tablets running Google 's Android software -LRB- logo pictured -RRB- has been revealed
...
` Installation of a Trojan application from the device manufacturer can grant the application full access to Android system and all applications -LRB- and their data -RRB- currently installed , ' wrote Forristal .

` The application then not only has the ability to read arbitrary application data on the device -LRB- email , SMS messages , documents , etc -RRB- , retrieve all stored account & service passwords , it can essentially take over the normal functioning of the phone and control any function thereof -LRB- make arbitrary phone calls , send arbitrary SMS messages , turn on the camera , and record calls -RRB- .

` Finally , and most unsettling , is the potential for a hacker to take advantage of the always-on , always-connected , and always-moving -LRB- therefore hard-to-detect -RRB- nature of these `` zombie '' mobile devices to create a botnet . '

Researchers say the loophole has been present since 2009 and claim around 900 million devices are at risk . One third party device , the Samsung Galaxy S4 -LRB- pictured right -RRB- , already has a fix for the flaw , but that Google 's own Nexus handset -LRB- left -RRB- does not
...
` Without that available to an attacker , this is likely to only be relevant for Android users who use third-party app stores -LRB- which have lots of other problems -RRB- . '
...
A new Android Trojan embedded in a pirated copy of an app -LRB- pictured -RRB- from rapper Jay Z has been identified .
