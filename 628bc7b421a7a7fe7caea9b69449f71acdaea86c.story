...
The device , known as KeySweeper , looks and works like a typical USB wall charger -LRB- pictured -RRB- , but uses a built-in chip to ` sniff ' for keystrokes typed onto nearby wireless keyboards . All of these strokes are then decrypted , logged and sent to a hacker over the web

KeySweeper was built by hardware hacker Samy Kamkar using an Arduino board , USB charger and Microsoft wireless keyboard .

These keyboards use a specific chip that runs on a frequency known as 2.4 GHz RF protocol , which is how the communicate wirelessly .
...
These keyboards use a specific chip that runs on a frequency known as 2.4 GHz RF protocol , which is how they communicate wirelessly .
...
The range of the KeySweeper is said to be on par with a standard Bluetooth device , at around 32ft -LRB- 10 metres -RRB- .
...
The logged , decrypted keystrokes are sent to a hacker remotely using the KeySweeper software -LRB- pictured -RRB- . The technology can be configured to record everything , and even send a text alert to the hacker if a certain combination of keys is pressed , such as the web address for online banking or a credit card number

Samy Kamkar -LRB- pictured bottom right -RRB- has released instructions -LRB- video grab pictured -RRB- for building a KeySweeper , but advises against people doing so without an electrical background . Mr Kamkar used Microsoft keyboards -LRB- pictured top -RRB- as an example because they use the same protocol and encryption
