...
German engineers have revealed designs to make 3D-printed ` bump keys ' -LRB- shown -RRB- . These devices can be used to break into popular pin tumbler design locks . In a video a lock is opened by a bump key and a hammer in seconds . To make the key all that is needed is a picture of the lock and pin positions
...
Lock bumping is a lock picking technique that can be used to open pin tumbler locks -LRB- shown above -RRB- , the sort found in many homes .
...
The technique for making bump keys is n't particularly new ; it 's been around for decades , with a hammer used to hit the key and click it into a lock 's pins .
...
` Basically , I take a photo -LSB- from the front -RSB- , then I manually edit it in Photoshop or Gimp -LRB- it 's really easy -RRB- to give me a black/white silhouette of the profile , ' Mr Holler continues .

` Once I have that , I import it into my software , specify the key length -LRB- that would also be guessed from the brand -RRB- and from that , I already get a 3D model that I can order from any online 3D printing service . '
...
To print them he does n't use his own printer , but rather a professional printing service which can take a few days , or up to two weeks .
...
The bump key is made from a 3D model of a blank -LRB- shown -RRB- and has groves that are all at the same level . When the key is placed in a lock , it needs to be struck sharply with a hammer while torque is applied . The force of the blow moves the pins and allows the lock to be turned and opened
...
But despite revealing his technique to create bump keys , Mr Holles does n't consider bumping ' a larger threat now ' with his work .

` The practical relevance of bumping -LRB- for burglars -RRB- seems to be overall low , so there is no reason to panic about this , ' he says .

` People also have n't panicked the last 10 years , although bumping was a well known technique . '
