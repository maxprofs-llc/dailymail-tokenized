...
Atmel has released its latest microcontrollers -LRB- MCUs -RRB- for a variety of gadgets that are so low power they can even harvest energy from a person 's body .

Atmel has released its latest microcontrollers -LRB- MCUs -RRB- for a variety of gadgets that are so low power they can even harvest energy from a person 's body . Atmel said its MCUs extend ` battery life from years to decades , reducing the number of times batteries need to be changed in devices '

The ultra-low power SAM L microcontrollers are part of Atmel 's 32-bit ARM-based MCU range .

These are all powered by chip maker ARM 's Cortex M0 + 32-bit processors .
...
This panel recognised a change in temperature between the hand and the room and this was harvested to create charge .
...
As Ars Technica 's Sean Gallagher explained , the processor is n't powerful enough to run a computer desktop , for example , but does come with enough power and memory to stream media from a USB stick , send alerts from smart alarms and lights and run multiple apps .
