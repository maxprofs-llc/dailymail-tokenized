...
Called HaptoMime , the system uses an imaging plate to transform an original image into a floating one , while infrared sensors tracks the position of fingers and an ultrasound machine creates tactile feedback . It is so advanced , people can enter multiple numbers on a keypad -LRB- left -RRB- or play a virtual piano -LRB- right -RRB-

The system was developed by Yasuaki Monnai and his team at the Shinoda-Makino Lab , University of Tokyo .

An LCD display generates an original image , which is then beamed onto an Aerial Imaging Plate -LRB- AIP -RRB- .
...
An LCD display generates an original image , which is then beamed onto an Aerial Imaging Plate -LRB- AIP -RRB- . To track fingers , the system is surrounded by an infrared touch sensor frame . When a person ` touches ' the floating image , an Ultrasound Phased Array Transducer creates the feeling of tactile feedback on the user 's finger

The technology was presented at the ACM Symposium on User Interface Software and Technology -LRB- UIST -RRB- in Hawaii .
...
The technology was presented at the ACM Symposium on User Interface Software and Technology -LRB- UIST -RRB- in Hawaii . It could be used to stop the spread of bacteria and germs on touchscreens , to enter PINs on ATMs , for example , or browse the web with wet hands

` It allows us to observe a screen floating in front of the mirror which is actually placed behind the mirror .

` Since all the visual effects such as binocular parallax and motion parallax are inherited to the floating screen , we can not distinguish it from a real screen until we touch it .

` Combined with a sensor that detects our finger motion , -LSB- HaptoMime -RSB- can be used as a virtual touch screen . '



@highlight

HaptoMime consists of an LCD , an Aerial Imaging Plate , infrared touch frame and an Ultrasound Phased Array Transducer
