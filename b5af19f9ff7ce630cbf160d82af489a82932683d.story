...
Coined the ` red-button attack ' - named after the red button used on modern smart TV remotes to access additional content - the flaw can be exploited with just a $ 250 -LRB- # 150 -RRB- transmitter .
...
Scientists at Columbia University have revealed how modern hyrbrid smart TVs -LRB- stock image shown -RRB- are vulnerable to an attack that was not previously known . Dubbed the ` red button attack ' , hackers could hijack broadcasts to access a viewer 's various connected internet accounts , including Facebook
...
The attack works by exploiting a vulnerability in Hybrid Broadcast-Broadband Television -LRB- HbbTV -RRB- .
...
This would be done by using a simple amplifier , costing as little as # 150 -LRB- $ 250 -RRB- on a rooftop to hijack networks across an area of 0.5 square miles -LRB- 1.4 square kilometres -RRB- .
...
The attack could be carried out by attaching a small and cheap transmitter to a drone -LRB- stock image shown -RRB- and then hovering outside a victim 's window . The attackers could also set up a transmitter on a roof to potentially hijack tens of thousands of TVs across an entire city
...
` This enables a large-scale exploitation technique with a localised geographical footprint based on radio frequency -LRB- RF -RRB- injection , ' the researchers continue .

This ` requires a minimal budget and infrastructure and is remarkably difficult to detect . '

` In a dense urban area , an attacker with a budget of about $ 450 -LRB- # 270 -RRB- can target more than 20,000 devices in a single attack . '
...
` This potential attack method is n't related specifically to the use of the red button on a TV remote specifically , but to any interaction with a smart TV , ' says David Emm , senior security researcher at Kaspersky Lab in Russia .
...
So-called hybrid TVs can be hijacked using a $ 250 -LRB- # 150 -RRB- 1-watt transmitter
