If you 've ever owned an iPhone and used iMessage , but switched to a non-Apple phone , you could be missing out on texts from friends and family .

Once iMessage is enabled , Apple routes messages from other iPhones through it . However , this can continue even when a number is switched to an Android phone , without access to the service .

As a result , if anyone sends an iMessage to that number it does n't get converted to a text , and ultimately ends up lost in the ether .

But now Apple has released a tool that will help people deregister their number from iMessage , or disable the service , to stop this from happening .

When a person registers an iPhone , their number is typically linked to iMessage . When they then swap to a new device , these messages are not always converted to text , and become lost . The tool -LRB- pictured -RRB- lets previous iPhone owners enter their phone number to deregister it from the service
...
But without access to iMessages , the recipient does n't receive it unless the sender resends it as a text .
...
The official Apple tool was spotted by a Reddit user
