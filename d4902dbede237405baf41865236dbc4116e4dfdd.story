...
LastPass ' Heartbleed Checker , pictured , looks to see when a site 's secure encryption certificate was last regenerated and warns if the server may be at risk . It also reveals the websites that either are n't affected by the bug , or have updated their certificates
...
The tool also reveals the websites that either are n't affected by the bug , or have updated their certificates .
...
OpenSSL is open-source software that is widely used to encrypt web communications . According to the tool , pictured , Bing.com does n't use OpenSSL so is not at risk
...
According to the tool , Bing.com does n't use OpenSSL so is not at risk .

The server details of Facebook and Twitter are n't reported , so LastPass was unable to confirm if they use OpenSSL .
...
` As providers rush to patch -LSB- the flaw -RSB- , consumers should apply typical IT security best practice : ensure you change passwords - once you know the issue has been fixed by your provider ; update your computers ; and do n't use the same password across multiple sites or services .

` This is not the first defect of its kind and it certainly wo n't be the last , but it is one of the more serious faults we 've seen in recent Internet history . '
