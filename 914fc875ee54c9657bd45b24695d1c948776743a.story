...
The malware is installed when the user downloads infected apps . When the power button is pressed , the malware shows a fake dialogue pop-up designed to look like the Android Power off menu -LRB- pictured -RRB- . The phone then appears to shut down and the malware can make calls , send messages and access other files
...
AVG has n't revealed which apps are carrying the malware and MailOnline has contacted the firm for more information .
...
The app runs in the background , so is n't immediately noticeable and wo n't appear as a rogue app or icon on the homescreen .
...
` This malware hijacks the shutting down process of your mobile , so when the user turns the power off button to shut down their mobile , it does n't really shut down , ' said the researchers .
...
The malware works by hijacking a line of code in the shutting down process and asking for root permission . This gives the hacker permission to access and modify any files -LRB- pictured -RRB- , apps and software on the device . But , running a scan using an antivirus app will detect and remove the files
