...
Betsy Davies -LRB- pictured -RRB- watched an online video tutorial before being asked to hack into a Wi-Fi hotspot . It took the seven-year-old 11 minutes to infiltrate the network by setting up a rogue access point - frequently used by attackers to activate a ` man in the middle ' attack , and begin eavesdropping on - or ` sniffing ' - traffic
...
To put these dangers into perspective , virtual private network -LRB- VPN -RRB- provider Hidemyass.com -LRB- HMA -RRB- recruited a child to attack a public network .
...
Unsecured Wi-Fi network : An unsecured Wi-Fi network is a wireless network that does n't request the user to log into it via the use of a username and password . These are usually displayed as OPEN networks .
...
WPA - Used Temporal Key Integrity Protocol -LRB- TKIP -RRB- : This protocol further improves the security of WEP without the need to buy new hardware . It still uses WEP for encryption , but it makes the attacks used to crack WEP a bit more difficult and time-consuming .

WPA2-PSK : This is the next level up from WPA and was designed for the home and small businesses . As the name implies , the new version uses a pre-shared key -LRB- PSK -RRB- . This has become standard that is now used by most households today .
...
RADIUS - Remote Authentication Dial In User Service -LRB- RADIUS -RRB- : A networking protocol that provides centralised Authentication , Authorisation , and Accounting management for users who connect and use a network service within a business . These users are authenticated against the company systems for added protection .

Channels : Wireless networks use Channels which are separated out so that various communication streams do n't interfere with each other . The 802.11 wireless standard allows for channels ranging from 1 thru 14 .
...
Virtual private network -LRB- VPN -RRB- : A VPN is a way of using the public internet like a secure private network . It encrypts data and routes it through remote servers , keeping the activity and location private and secure .

Rogue Access Points are wireless access points , installed on a company 's network without the company 's knowledge . Ms Davies began by searching for , and watching , a freely available video tutorial detailing how to hack a network - a Google search returns over 11 million results , and YouTube lists 14,000 tutorials -LRB- pictured -RRB-
...
According to follow-up research , nearly two thirds -LRB- 59 per cent -RRB- of people regularly use unsecure , or ` open ' Wi-Fi hotspots with one in five -LRB- 20 per cent -RRB- doing so weekly or more . Among those who use these unsecured networks , 19 per cent log on to online banking and 31 per cent send emails and personal documents
...
Use a Virtual Private Network -LRB- VPN -RRB- : Not all sites display the Https lock symbol , but a VPN will act as an intermediary between the device and the internet server .

Downloading an app ? Check permissions : Before installing , make sure apps ca n't access unnecessary information . For example , a drawing app should n't need a contacts list .

Forget the network : When leaving a publich network , do n't just log off , ask the device to forget the network so it does n't automatically log on when in range later .
...
According to follow-up research by HMA , nearly two thirds -LRB- 59 per cent -RRB- of people regularly use unsecure , or ` open ' Wi-Fi hotspots with one in five -LRB- 20 per cent -RRB- doing so weekly or more .
