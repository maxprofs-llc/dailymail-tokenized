...
According to WESH.com , the mother has straightened her life out since the 1991 arrest , graduating college with a degree in architecture . She now works for Pratt & Whitney , a jet design firm , in Connecticut .
...
` I just want it to be all over so I can go home , ' she told NBC Connecticut from her holding cell in Brevard County Jail .

Because of the holiday weekend , Hall may be held until next Thursday .

var nbcLP ={ -RCB- ; nbcLP.aRandomNumber = Math.floor -LRB- Math.random -LRB- -RRB- * 10000 -RRB- ; nbcLP.currentPageLoc = encodeURIComponent -LRB- window.location.href -RRB- ; nbcLP.currentSiteLoc = encodeURIComponent -LRB- window.location.host -RRB- ; nbcLP.defaultWidth = 652 ; nbcLP.defaultHeight = 367 ; nbcLP.cmsID = `` 187829481 '' ; nbcLP.vidPid = `` uUeubsVds68v '' ; nbcLP.vidSec = `` news '' ; nbcLP.vidSubSec = `` local '' ; nbcLP.vidFrame = document.getElementById -LRB- `` nbcLP187829481 '' -RRB- ; nbcLP.vidFrame.style.border = `` none '' ; nbcLP.vidFrame.width = nbcLP.defaultWidth ; nbcLP.vidFrame.height = nbcLP.defaultHeight ; nbcLP.vidFrame.scrolling = `` no '' ; nbcLP.vidFrame.src = `` http://www.nbcconnecticut.com/templates/nbc_partner_player?cmsID= '' + nbcLP.cmsID + '' & videoID = '' + nbcLP.vidPid + '' & width = '' + nbcLP.defaultWidth + '' & height = '' + nbcLP.defaultHeight + '' & sec = '' + nbcLP.vidSec + '' & subsec = '' + nbcLP.vidSubSec + '' & turl = '' + nbcLP.currentSiteLoc + '' & ourl = '' + nbcLP.currentPageLoc + '' & rand = '' + nbcLP.aRandomNumber ;
