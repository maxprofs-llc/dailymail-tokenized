...
The Deep Dense Face Detector algorithm was built by Yahoo Labs in California and Stanford University . The researchers used a form of machine learning known as a deep convolutional neural network to train a computer to spot facial features -LRB- pictured -RRB- in a database of images

At the moment , the so-called Deep Dense Face Detector does n't recognise who the individual faces belong to , just that there is a face .
...
Mr Farfade trained his algorithm using a database of 200,000 images featuring faces shown at various angles and orientations , plus 20 million images that did n't contain faces .
...
` It has minimal complexity ... and can get similar or better performance -LSB- than other systems -RSB- while it does not require annotation or information about facial landmarks . '

And , the team said the technology could be improved following further training .

The algorithm -LRB- recognition pictured left -RRB- can identify faces from various different angles , when part of the face is hidden and even upside down -LRB- example pictured right -RRB- . At the moment , the technology does n't recognise who a face belongs to , but could be trained to do so

Google used a similar neural network technique for its recent GoogLeNet classification algorithm that can identify images within images , such as a hat on the head of a dog -LRB- pictured -RRB-
...
Google 's technology is so accurate it can locate and distinguish between a range of object sizes within a single image , and it can also determine an object within , or on top of , an object , within the photo -LRB- pictured -RRB-

Facebook 's Deep Face tool also used this neural network technique to help recognise users in photos . Its algorithm identifies faces ` as accurately as a human ' and uses a 3D model to virtually rotate faces so they are facing the camera . Image -LRB- a -RRB- shows the original image and -LRB- g -RRB- shows the final , corrected version
...
Facebook uses Deep Face to offer tag suggestions which the user can accept , or reject . The tagging option -LRB- pictured bottom -RRB- has started appearing in the settings of accounts globally - although in many cases it says the feature is ` unavailable ' - and a number of users have reportedly been given the tool
...
It does n't recognise who a face belongs to , but could be trained to do so
